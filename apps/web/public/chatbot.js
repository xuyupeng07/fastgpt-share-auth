function embedChatbot() {
  ...
  console.log('embedChatbot 函数开始执行');
  console.log('botSrc:', botSrc);
  console.log('script element:', script);

  if (!botSrc) {
    console.error(`Can't find botSrc`);
    return;
  }
  if (document.getElementById(chatBtnId)) {
    console.log('聊天按钮已存在，跳过创建');
    return;
  }

  console.log('开始创建聊天按钮和窗口');
...
// 暴露到全局作用域
window.embedChatbot = embedChatbot;

console.log('chatbot.js 脚本已加载');